...
abstract class SanitizeVars {
    ...
    public static function STRING_NORMAL($string, $min='', $max=''){
    ...
    public static function STRING_LETRAS_NUMEROS($string, $min='', $max=''){
   ...
   //VALIDA SIN REEMPLAZAR
    ...
    //VALIDA SIN REEMPLAZAR
    public static function APELLIDONOMBRES($string, $min='', $max=''){
        if (preg_match("/^[a-zA-ZñÑ áéíóúÁÉÍÓÚ]*$/", $string)) {
        $len = strlen($string);
        if((($min != '') && ($len < $min)) || (($max != '') && ($len > $max)))
        return FALSE;
        return $string;
    ...
    public static function CODIGO_ORGANISMO($string, $min='', $max=''){
